...
class HrAttendance(http.Controller):
    ...
    @http.route('/save/check_in_image', type='http', auth='user')
    def save_check_in_image(self, **post):
        # Retrieve the image data from the request
        image_data = post.get('image_data')
        
        # Decode the Base64-encoded image data (you may need to import base64)
        decoded_image = base64.b64decode(image_data.split(',')[1])
        
        # Retrieve the current attendance record
        attendance_id = int(post.get('attendance_id'))
        attendance = request.env['hr.attendance'].sudo().browse(attendance_id)
        
        # Save the decoded image data to the check_in_image field of the attendance record
        attendance.check_in_image = decoded_image
        
        # Optionally, you can return a response to the client indicating success or failure
