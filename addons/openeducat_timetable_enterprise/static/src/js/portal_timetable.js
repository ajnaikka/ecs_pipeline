odoo.define('openeducat_timetable_enterprise.portal_timetable', function (require) {
    ...
    var PortalTimeTableWidget = Widget.extend({
        jsLibs: [
        ...
        InitKendo: async function(){
            ...
            ajax.jsonRpc("/get-timetable/data",'call',{
                ...
                $('#timetable_portal_kendo').kendoScheduler({
                    ...
                    edit: function(e) {
                        e.container.find(".k-scheduler-update").text('Close');
                        e.container.find(".k-scheduler-cancel").hide();
                        e.container.parent().find(".k-window-actions").hide();
                    ...
                    dataBound: function(e) {
                        $('.k-state-default.k-nav-current').click( function(){
                            setTimeout(() => {
                                $('.k-calendar-container.k-popup.k-group.k-reset').css('display', 'block')
                            }, 200);
                        });
                    },
                });
