...
class StudentExamInvite(models.TransientModel):
    _name = 'student.exam.invite'
    _inherit = 'mail.composer.mixin'
    _description = 'Student Exam Invitation Wizard'
    ...
    attachment_ids = fields.Many2many(
        'ir.attachment', 'student_exam_mail_compose_message_ir_attachments_rel', 'wizard_id', 'attachment_id',
        string='Attachments')
    # origin
    ...
    partner_ids = fields.Many2many(
        'res.partner', 'student_exam_invite_partner_ids', 'invite_id', 'partner_id', string='Recipients',

        domain="[('id', 'in', exam_attendees_ids), '|', (survey_users_can_signup, '=', 1), '|', (not survey_users_login_required, '=', 1), ('user_ids', '!=', False)]"
    )
    existing_partner_ids = fields.Many2many(
        'res.partner', compute='_compute_existing_partner_ids', readonly=True, store=False)
    exam_attendees_ids = fields.Many2many(
        'res.partner', compute='_compute_exam_attendees', readonly=True, store=False)
    ...
    exam_invite_attempt = fields.Selection([('1', 'First'), ('2', 'Second'), ('3', 'Third')], string="No of Exam Invite",
                                    required=True)

    @api.depends('partner_ids', 'exam_id')
    def _compute_exam_attendees(self):
        self.exam_attendees_ids = list(set(self.exam_id.attendees_line.student_id.partner_id.ids))
    ...
    def _compute_subject(self):
        for invite in self:
            ...
            super(StudentExamInvite, invite)._compute_subject()
    ...
    def _compute_body(self):
        for invite in self:
            ...
            if len(langs) == 1:
                invite = invite.with_context(lang=langs.pop())
            super(StudentExamInvite, invite)._compute_body()
    ...
    def _send_mail(self, answer):
        ...
        if self.exam_invite_attempt == '3':
            self.exam_id.survey_id_3_shared = True
        elif self.exam_invite_attempt == '2':
            self.exam_id.survey_id_2_shared = True
        else:
            self.exam_id.survey_id_shared = True
        return created_mail.send()
    ...
    def action_invite(self):
        ...
        for answer in answers:
            if answer.partner_id and answer.partner_id in self.exam_attendees_ids:
                attendees_line = self.exam_id.attendees_line.filtered(lambda l: l.student_id.partner_id.id == answer.partner_id.id)
                attendees_line.survey_user_input_id = answer.id
                answer.exam_attendees_id = attendees_line.id
