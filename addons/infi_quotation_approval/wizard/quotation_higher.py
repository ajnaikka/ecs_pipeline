...
class QuotationApprovalHigher(models.TransientModel):
    _name = 'quotation.higher'
    ...
    group_purchase_director = fields.Many2one('res.groups', default=lambda self: self.env.ref('purchase_order_approval.group_technical_team', raise_if_not_found=False))
    ...
    attachment_sec_ids = fields.Many2many('ir.attachment', 'quotation_higher_attachment_rel_2', 'wizard_id', 'attachment_id', string="Attachments 2")
    ...
    def to_cc(self):
        for wizard in self:
            ...
            email_to_list = [user.email for user in wizard.send_to]
