...
class OpBatch(models.Model):
    ...
    course_ids = fields.Many2many('op.course','batch_id',string="Courses")
    ...
    def action_confirm(self):
        for data in self:
            ...
            for rec in data.course_ids:
                print(rec.name)
                normal_exam = {'name':rec.name +'-'+data.name+'- (Normal)',
                               'exam_code':'N-'+data.code+'-'+rec.code+'-'+str(normal_year)+'-'+str(normal_month),
                ...
                emergency_exam = {'name':rec.name +'-'+data.name+'- (Emergency)',
                               'exam_code':'E-'+data.code+'-'+rec.code+'-'+str(emergency_year)+'-'+str(emergency_month),
                ...
                super_exam = {'name':rec.name +'-'+data.name+'- (Super Emergency)',
                               'exam_code':'S-'+data.code+'-'+rec.code+'-'+str(super_year)+'-'+str(super_month),
