...
class Batchcalendar(models.Model):
    ...
    @api.depends('course_id')
    def onchange_batch_course_id(self):
        for rec in self:
            rec.calendar_lines = False
            batches = self.env['op.batch'].search([('course_id', '=', rec.course_id.id)])
            data = []
    ...
    is_compute = fields.Boolean(compute='onchange_batch_course_id')
